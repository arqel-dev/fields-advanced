...
**Entregue (FIELDS-ADV-001..007):**

- Esqueleto do pacote (`composer.json`, PSR-4 `Arqel\FieldsAdvanced\` → `src/`, deps em `arqel/core` + `arqel/fields` via path repos)
- `FieldsAdvancedServiceProvider` (final, auto-discovered) registra `richText`, `markdown`, `code`, `repeater`, `builder` e `keyValue` no `FieldFactory`
...
- **91 testes Pest passando** — 8 ServiceProvider (todas 6 macros: `richText`, `markdown`, `code`, `repeater`, `builder`, `keyValue`) + 10 RichTextField + 11 MarkdownField + 12 CodeField + 15 RepeaterField + 5 Block + 16 BuilderField + 14 KeyValueField (construção via `new`/`make()`/macro, defaults, fluência, empty key/value label throws, placeholders aceitam vazio, toggles individuais, `asObject` 3 estados, payload completo de 9 keys end-to-end)

**Por chegar (FIELDS-ADV-008..010):**
