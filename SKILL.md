...
**Entregue (FIELDS-ADV-001..006):**

- Esqueleto do pacote (`composer.json`, PSR-4 `Arqel\FieldsAdvanced\` → `src/`, deps em `arqel/core` + `arqel/fields` via path repos)
- `FieldsAdvancedServiceProvider` (final, auto-discovered) registra `richText`, `markdown`, `code`, `repeater` e `builder` no `FieldFactory`
...
- **76 testes Pest passando** — 7 ServiceProvider (todas 5 macros: `richText`, `markdown`, `code`, `repeater`, `builder`) + 10 RichTextField + 11 MarkdownField + 12 CodeField + 15 RepeaterField + 5 Block (toArray shape, default icon null, schema é Field[]) + 16 BuilderField (construção, defaults, `blocks` 3 modos, filter silent, duplicate type throws, clamps, toggles, payload completo)

**Por chegar (FIELDS-ADV-007..010):**
